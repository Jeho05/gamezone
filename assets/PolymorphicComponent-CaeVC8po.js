import{r as g}from"./react-vendor-Ct9RIpkg.js";if(typeof window<"u"){const r={NEXT_PUBLIC_API_BASE:"https://votre-domaine.infinityfreeapp.com/api",NEXT_PUBLIC_KKIAPAY_PUBLIC_KEY:"072b361d25546db0aee3d69bf07b15331c51e39f",NEXT_PUBLIC_KKIAPAY_SANDBOX:"0"};globalThis.process??={};const t=globalThis.process.env??{};globalThis.process.env=new Proxy(Object.assign({},r,t),{get(n,s){return s in n?n[s]:void 0},has(){return!0}})}const m="data-render-id";function u(r,t){const n=Math.max(r,t),s=`
...
`;return`data:image/svg+xml;utf8,${encodeURIComponent(s)}`}function v(r){const t=g.useRef(null);return r&&"instance"in r?t:r??t}const E=g.forwardRef(function({as:t,children:n,renderId:s,onError:k,...y},f){const p=t==="img"?{...y,onError:e=>{typeof k=="function"&&k(e);const o=e.currentTarget,{width:c,height:a}=o.getBoundingClientRect();o.dataset.hasFallback="1",o.onerror=null,o.src=u(Math.round(c)||128,Math.round(a)||128),o.style.objectFit="cover"}}:y,i=v(f);return g.useEffect(()=>{const e=i&&"current"in i?i.current:null;if(!e)return;if(t!=="img"){const c=()=>{const{width:l,height:h}=e.getBoundingClientRect();return u(Math.round(l)||128,Math.round(h)||128)},a=()=>{e.dataset.hasFallback="1",e.style.backgroundImage=`url("${c()}")`,e.style.backgroundSize="cover"},d=()=>{const l=getComputedStyle(e).backgroundImage,C=/url\(["']?(.+?)["']?\)/.exec(l)?.[1];if(!C)return;const w=new Image;w.onerror=a,w.src=C};d();const x=new ResizeObserver(([l])=>{if(!e.dataset.hasFallback)return;const{width:h,height:C}=l.contentRect;e.style.backgroundImage=`url("${u(Math.round(h)||128,Math.round(C)||128)}")`});x.observe(e);const b=new MutationObserver(d);return b.observe(e,{attributes:!0,attributeFilter:["style","class"]}),()=>{x.disconnect(),b.disconnect()}}if(!e.dataset.hasFallback)return;const o=new ResizeObserver(([c])=>{const{width:a,height:d}=c.contentRect;e.src=u(Math.round(a)||128,Math.round(d)||128)});return o.observe(e),()=>o.disconnect()},[t,i]),g.createElement(t,Object.assign({},p,{ref:i,...s?{[m]:s}:void 0}),n)});export{E as C};
